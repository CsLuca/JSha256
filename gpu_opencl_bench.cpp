...
void double_sha256_header(const __global uchar* header80, uint nonce, __private uint out_digest[8]) {
...
__kernel void g1_bench(__global const uchar* header80, uint nonce_base, uint iter, __global uint* out) {
    ...
    if (idx >= iter) return;
    uint digest[8];
    double_sha256_header(header80, nonce_base + idx, digest);
    out[idx] = digest[0];
}
...
__kernel void g1_check(__global const uchar* header80, __global const uint* nonces, uint count, __global uint* out_words) {
    ...
    if (idx >= count) return;
    uint digest[8];
    double_sha256_header(header80, nonces[idx], digest);
...
int main(int argc, char** argv) {
    ...
    cl_kernel kernel_bench = clCreateKernel(program, "g1_bench", &st);
    if (!check_status(st, "clCreateKernel(g1_bench)")) return 3;
    cl_kernel kernel_check = clCreateKernel(program, "g1_check", &st);
    ...
    if (options.run_check) {
        ...
        cl_mem d_words = clCreateBuffer(context, CL_MEM_WRITE_ONLY,
                                        kKnownCount * 8 * sizeof(uint32_t), nullptr, &st);
        if (!check_status(st, "clCreateBuffer(words)")) return 4;

        const uint32_t count_u32 = static_cast<uint32_t>(kKnownCount);
        st = clSetKernelArg(kernel_check, 0, sizeof(cl_mem), &d_header);
        st |= clSetKernelArg(kernel_check, 1, sizeof(cl_mem), &d_nonces);
        st |= clSetKernelArg(kernel_check, 2, sizeof(uint32_t), &count_u32);
        st |= clSetKernelArg(kernel_check, 3, sizeof(cl_mem), &d_words);
        ...
        st = clEnqueueNDRangeKernel(queue, kernel_check, 1, nullptr, &global, &local, 0, nullptr, nullptr);
        if (!check_status(st, "clEnqueueNDRangeKernel(g1_check)")) return 4;
        st = clFinish(queue);
        if (!check_status(st, "clFinish(g1_check)")) return 4;

        std::vector<uint32_t> words(kKnownCount * 8, 0);
        st = clEnqueueReadBuffer(queue, d_words, CL_TRUE, 0,
                                 words.size() * sizeof(uint32_t), words.data(), 0, nullptr, nullptr);
        ...
        if (!ok) {
            std::fprintf(stderr, "Correctness check failed for OpenCL G1\n");
            return 4;
        }
    }
    ...
    st = clSetKernelArg(kernel_bench, 0, sizeof(cl_mem), &d_header);
    st |= clSetKernelArg(kernel_bench, 1, sizeof(uint32_t), &nonce_base);
    st |= clSetKernelArg(kernel_bench, 2, sizeof(uint32_t), &iter_u32);
    st |= clSetKernelArg(kernel_bench, 3, sizeof(cl_mem), &d_out);
    ...
    st = clEnqueueNDRangeKernel(queue, kernel_bench, 1, nullptr, &global, &local, 0, nullptr, nullptr);
    ...
    const double hs = static_cast<double>(options.iter) / safe_seconds;
    const double ns = (safe_seconds * 1e9) / static_cast<double>(options.iter);
    std::printf("G1,%.6f,%.6f\n", hs, ns);

    clReleaseMemObject(d_out);
    clReleaseMemObject(d_header);
    clReleaseKernel(kernel_check);
    clReleaseKernel(kernel_bench);
